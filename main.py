...
def book_slot(data: BookingModel):
    ...
    if host and "user_id" in host:
        host_telegram_id = int(host["user_id"])
        keyboard = {
            "inline_keyboard": [
                [
...
def get_user_bookings(user_id: int):
    user_bookings = list(bookings_col.find({"user_id": user_id}, {"_id": 0}))
...
async def recharge(user_id: int = Form(...), amount_inr: int = Form(...), utr_number: str = Form(...), screenshot: UploadFile = File(...)):
    ...
    recharges_col.insert_one({"recharge_id": recharge_id, "user_id": user_id, "amount_inr": amount_inr, "tokens_expected": tokens_expected, "utr_number": utr_number, "status": "pending"})
...
def withdraw_earnings(data: WithdrawModel):
    ...
    withdrawals_col.insert_one({"user_id": data.user_id, "upi_id": data.upi_id, "tokens": data.tokens, "status": "pending"})
    ...
    if "message" in body:
        ...
        if data_str.startswith("approve_rc_"):
            ...
            if booking:
                ...
                send_telegram_message(booking["user_id"], "🎉 <b>Host accepted your booking!</b> Go to 'My Bookings' in the app to join.")
            ...
            if booking:
                ...
                users_col.update_one({"user_id": booking["user_id"]}, {"$inc": {"tokens": booking["token_cost"]}})
                send_telegram_message(booking["user_id"], f"❌ Booking rejected. {booking['token_cost']} tokens refunded.")
