...
def get_user_data(user_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT name, phone, balance, referred_by FROM users WHERE user_id = ?",
        (user_id,),
    )
    row = cursor.fetchone()
    conn.close()
...
def is_new_user(user_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT COUNT(*) FROM sessions WHERE user_id = ? AND duration > 0",
        (user_id,),
    )
...
def log_session(user_id, host_name, duration):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "INSERT INTO sessions (user_id, host_name, duration) VALUES (?, ?, ?)",
        (user_id, host_name, duration),
    )
    conn.commit()
    conn.close()
...
def get_user_plan(user_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT plan_text FROM user_plans WHERE user_id = ?", (user_id,)
    )
    row = cursor.fetchone()
    conn.close()
...
def get_pending_txn(txn_id):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT user_id, utr, msg_id, status FROM pending_txns WHERE txn_id = ?",
        (txn_id,),
    )
    row = cursor.fetchone()
    conn.close()
...
def deduct_host_payout_mins(host_user_id, mins):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT slot_name FROM host_assignments WHERE user_id = ?",
        (host_user_id,),
    )
    row = cursor.fetchone()
    if row:
...
def get_host_worked_mins(host_user_id):
    ...
    cursor.execute(
        "SELECT SUM(duration) FROM sessions WHERE host_name = ?", (slot_name,)
    )
...
def remove_host_slot(identifier):
    ...
    if str(identifier).isdigit():
        cursor.execute(
            "DELETE FROM host_assignments WHERE user_id = ?",
            (int(identifier),),
        )
    else:
        cursor.execute(
            "DELETE FROM host_assignments WHERE slot_name = ?", (identifier,)
        )
    conn.commit()
    conn.close()
...
def update_host_status(user_id, status):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "UPDATE host_assignments SET status = ? WHERE user_id = ?",
        (status, user_id),
    )
...
REGISTERED_USER_GROUP_ID = int(
    os.environ.get("REGISTERED_USER_GROUP_ID", "-1003698246938")
)
...
def broadcast_cmd(message):
    ...
    if not msg_text:
        bot.send_message(
            ADMIN_GROUP_ID, "⚠️ Usage: `/broadcast <Aapka Message>`"
        )
        return
...
def stats_cmd(message):
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT COUNT(*), SUM(duration) FROM sessions WHERE duration > 0"
    )
    row = cursor.fetchone()
    conn.close()
...
def admin_manual_withdraw(message):
    ...
    if len(args) < 3 or not args[1].isdigit() or not args[2].isdigit():
        bot.send_message(
            ADMIN_GROUP_ID,
            "⚠️ Usage: `/adminwithdraw <host_user_id> <amount_inr>`\nExample: `/adminwithdraw 123456789 1000`",
        )
        return
...
def set_host_direct(message):
    ...
    if slot_name not in HOST_GROUPS:
        bot.send_message(
            message.chat.id,
            "❌ *Invalid Slot Name!* Kripya sahi slot name likhein.",
        )
        return
...
def delete_host_direct(message):
    ...
    if len(args) < 2:
        bot.send_message(
            message.chat.id, "⚠️ Usage: `/delhost <user_id or Slot Name>`"
        )
        return
    ...
    bot.send_message(
        ADMIN_GROUP_ID,
        f"🗑️ *HOST REMOVED SUCCESSFULLY!*\nTarget `{target}` ko unassign kar diya gaya hai.",
    )


# --- 6. HOST BALANCE & EARNINGS ---
@bot.message_handler(
    func=lambda msg: msg.text
    in ["👑 Host Earnings", "Host Earnings", "/balance", "/earnings"]
)
def check_balance_earnings(message):
    ...
    markup.add(
        types.InlineKeyboardButton(
            "🟢 Go Online", callback_data="host_cmd_online"
        ),
        types.InlineKeyboardButton(
            "🔴 Go Offline", callback_data="host_cmd_offline"
        ),
    )
    markup.add(
        types.InlineKeyboardButton(
            "💸 Withdraw (₹700 - ₹3000)", callback_data="action_withdraw"
        )
    )

    msg = "👑 *HOST DASHBOARD & EARNINGS*\n─────────────────────────\n"
...
@bot.callback_query_handler(
    func=lambda call: call.data in ["host_cmd_online", "host_cmd_offline"]
)
def inline_host_status_change(call):
    ...
    if call.data == "host_cmd_online":
        ...
        bot.answer_callback_query(
            call.id, "🔴 Status: OFFLINE", show_alert=True
        )


# --- 7. BOOK HOST SESSION ---
@bot.message_handler(
    func=lambda msg: msg.text in ["Book Host Session", "🔥 Book Host Session"]
)
        ...
        if slot_name in status_map:
            ...
            if current_status == "ONLINE":
                markup.add(
                    types.InlineKeyboardButton(
                        f"🟢 {slot_name} (Online)",
                        callback_data=f"select_host_{slot_name}",
                    )
                )
            else:
                markup.add(
...
def alert_host_offline(call):
    bot.answer_callback_query(
        call.id,
        "⚠️ Yeh Host abhi offline hai. Kripya Online Host chunnein!",
        show_alert=True,
    )


@bot.callback_query_handler(
    func=lambda call: call.data.startswith("select_host_")
)
def show_host_durations(call):
    ...
    if is_new_user(user_id):
        markup.add(
            types.InlineKeyboardButton(
                "🎁 1 Min (New User Offer - ₹20)",
                callback_data=f"book_{host_name}_1",
            )
        )

    markup.add(
        types.InlineKeyboardButton(
            "⏱️ 5 Mins", callback_data=f"book_{host_name}_5"
        ),
        types.InlineKeyboardButton(
            "⏱️ 10 Mins", callback_data=f"book_{host_name}_10"
        ),
        types.InlineKeyboardButton(
            "⏱️ 20 Mins", callback_data=f"book_{host_name}_20"
        ),
        types.InlineKeyboardButton(
            "⏱️ 30 Mins", callback_data=f"book_{host_name}_30"
        ),
    )
...
def process_booking_click(call):
    ...
    if user_info["balance"] < mins:
        bot.answer_callback_query(
            call.id,
            f"❌ Balance Kam Hai! {mins} Mins chahiye.",
            show_alert=True,
        )
        return
    ...
    if not invite_link:
        bot.send_message(
            user_id, "⚠️ Technical Issue ke karan link generate nahi ho paya."
        )
        return
    ...
    link_markup.add(
        types.InlineKeyboardButton(
            "🔗 Join Private Session Now", url=invite_link
        )
    )
...
@bot.message_handler(
    func=lambda msg: msg.text in ["💳 Recharge Minutes", "Recharge Minutes"]
)
def buy_minutes(message):
    ...
    if is_new_user(user_id):
        markup.add(
            types.InlineKeyboardButton(
                "🎁 ₹20 — 1 Minute (New User Trial)",
                callback_data="payplan_20_1",
            )
        )

    markup.add(
        types.InlineKeyboardButton(
            "⭐ ₹100 — 5 Minutes", callback_data="payplan_100_5"
        ),
        types.InlineKeyboardButton(
            "🔥 ₹200 — 10 Minutes", callback_data="payplan_200_10"
        ),
        types.InlineKeyboardButton(
            "🚀 ₹400 — 20 Minutes", callback_data="payplan_400_20"
        ),
        types.InlineKeyboardButton(
            "💎 ₹600 — 30 Minutes", callback_data="payplan_600_30"
        ),
    )
    bot.send_message(
        message.chat.id, "💳 *SELECT RECHARGE PLAN*", reply_markup=markup
    )
...
def handle_screenshot(message):
    if message.chat.type == "private":
        ...
        bot.register_next_step_handler(
            msg, process_utr_submission, photo_id, txn_id
        )
...
def process_utr_submission(message, photo_id, txn_id):
    ...
    markup.add(
        types.InlineKeyboardButton(
            "1 Min (Rs.20)", callback_data=f"app_{user_id}_1_{txn_id}"
        ),
        types.InlineKeyboardButton(
            "5 Min (Rs.100)", callback_data=f"app_{user_id}_5_{txn_id}"
        ),
        types.InlineKeyboardButton(
            "10 Min (Rs.200)", callback_data=f"app_{user_id}_10_{txn_id}"
        ),
        types.InlineKeyboardButton(
            "20 Min (Rs.400)", callback_data=f"app_{user_id}_20_{txn_id}"
        ),
        types.InlineKeyboardButton(
            "30 Min (Rs.600)", callback_data=f"app_{user_id}_30_{txn_id}"
        ),
        types.InlineKeyboardButton(
            "❌ Reject", callback_data=f"rej_{user_id}_{txn_id}"
        ),
    )
    caption = (
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith(("app_", "rej_"))
)
def process_admin_recharge_approval(call):
    ...
    if action == "app":
        ...
        if referrer_id > 0:
            ...
            cursor = conn.cursor()
            cursor.execute(
                "UPDATE users SET referred_by = 0 WHERE user_id = ?",
                (user_id,),
            )
            conn.commit()
            conn.close()
            ...
            try:
                bot.send_message(
                    referrer_id,
                    "🎉 *REFERRAL BONUS RECEIVED!*\n\nAapke referred friend ne pehla recharge successfully complete kar liya hai. Aapko **1 Minute** free bonus mil gaya hai!",
                )
            except Exception:
                pass
...
# --- 9. PROFILE, REFERRAL & WITHDRAWAL ---
@bot.message_handler(
    func=lambda msg: msg.text in ["Profile & Referral", "👤 Profile & Referral"]
)
def profile_and_ref(message):
    ...
    if slot_name:
        ...
        markup.add(
            types.InlineKeyboardButton(
                "💸 Host Withdrawal (₹700-₹3000)",
                callback_data="action_withdraw",
            )
        )
...
@bot.message_handler(
    func=lambda msg: msg.text in ["Help & Support", "🆘 Help & Support"]
)
def help_and_tutorial(message):
    markup = types.InlineKeyboardMarkup(row_width=2)
    markup.add(
        types.InlineKeyboardButton(
            "🎬 Watch Tutorial", callback_data="show_tutorial"
        ),
        types.InlineKeyboardButton(
            "💬 Contact Support", url="https://t.me/VynoraSupport"
        ),
    )
...
@bot.callback_query_handler(
    func=lambda call: call.data in ["show_tutorial", "action_withdraw"]
)
...
def start_withdrawal(message):
    ...
    bot.register_next_step_handler(
        msg, process_withdraw_amount, net_balance_inr, worked_mins
    )
...
def process_withdraw_amount(message, net_balance_inr, worked_mins):
    ...
    if (
        not amount_text.isdigit()
        or int(amount_text) < 700
        or int(amount_text) > 3000
        or int(amount_text) > net_balance_inr
    ):
        msg = bot.send_message(
            user_id,
    ...
    bot.register_next_step_handler(
        msg, process_withdraw_upi, amount_inr, mins_equivalent
    )
...
def process_withdraw_upi(message, amount_inr, mins_equivalent):
    ...
    markup.add(
        types.InlineKeyboardButton(
            "✅ Approve & Mark Paid",
            callback_data=f"payout_done_{user_id}_{amount_inr}_{mins_equivalent}",
        ),
        types.InlineKeyboardButton(
            "❌ Reject", callback_data=f"payout_reject_{user_id}"
        ),
    )
...
@bot.callback_query_handler(
    func=lambda call: call.data.startswith(("payout_done_", "payout_reject_"))
)
def admin_payout_action(call):
    ...
    if action == "payout" and parts[1] == "done":
        user_id, amount_inr, mins_equivalent = (
            int(parts[2]),
            int(parts[3]),
            int(parts[4]),
        )
        ...
        bot.send_message(
            user_id,
            "❌ Aapka withdrawal request admin dwara reject kar diya gaya hai.",
        )
