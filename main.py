...
# Admin IDs (Including your new Admin ID 1108685585 and primary admin)
ADMIN_IDS = [1108685585, 999999999]  # Add other admin IDs here if needed
...
def validate_telegram_init_data(init_data: str) -> bool:
    """Validates Telegram WebApp initData for security."""
    try:
        ...
        return True  # Fallback for local testing if needed
...
@app.get("/api/hosts")
def get_hosts():
    # Real approved hosts first
    db_hosts = list(hosts_col.find({"status": "approved"}, {"_id": 0}))
    for h in db_hosts:
        h["loc"] = "India"  # All real creators tagged as India as requested
        h["is_dummy"] = False

    # 5 Dummy Hosts (Non-bookable with polite busy message, placed at the bottom)
    dummy_hosts = [
        {"id": "dummy_1", "user_id": 9991, "name": "Sophia 💎", "age": 22, "rate": 40, "lang": "English", "loc": "UK (International)", "img": "https://images.unsplash.com/photo-1494790108377-be9c29b29330?w=400&auto=format&fit=crop", "bio": "International VIP Model ✨", "is_online": True, "is_dummy": True},
        ...
        {"id": "dummy_3", "user_id": 9993, "name": "Elena 👑", "age": 23, "rate": 60, "lang": "French", "loc": "France (International)", "img": "https://images.unsplash.com/photo-1524504388940-b1c1722653e1?w=400&auto=format&fit=crop", "bio": "Parisian fashion enthusiast 🌸", "is_online": True, "is_dummy": True},
        {"id": "dummy_4", "user_id": 9994, "name": "Natasha ✨", "age": 20, "rate": 45, "lang": "Russian", "loc": "Russia (International)", "img": "https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=400&auto=format&fit=crop", "bio": "Professional singer & artist 🎶", "is_online": True, "is_dummy": True},
...
def book_slot(data: BookingModel):
    if str(data.host_id).startswith("dummy_"):
        return {"status": "error", "message": "✨ Host is currently busy in a private international session (Different Country). Please try another host!"}
...
def accept_booking(data: ActionBookingModel):
    ...
    current_time = time.time()
    # Timer will start accurately when call is connected/accepted
...
def complete_booking(data: CompleteBookingModel):
    ...
    if booking:
        ...
        # Send detailed call history report to Group 3 as requested
        ...
        return {"status": "success", "message": "Booking completed and history sent to Group 3"}
    return {"status": "error", "message": "Booking not found"}
...
def withdraw_earnings(data: WithdrawModel):
    ...
    user = users_col.find_one({"user_id": int(data.user_id)})
    net_earnings = int(user.get("earnings", 0) * 0.7) if user else 0
...
async def telegram_webhook(req: Request):
    ...
    if "message" in body:
        ...
        user_first_name = msg["from"].get("first_name", "User")
        username = msg["from"].get("username", "N/A")
        user_display = f"{user_first_name} (@{username})"

        # ADMIN COMMANDS CHECK (Only allowed for ADMIN_IDS)
        ...
        if text.startswith("/ban "):
            ...
            try:
                ...
                if user:
                    ...
                    msg_text = f"❌ User ID <code>{target_id}</code> database mein nahi mila!"
            ...
            try:
                ...
                host = hosts_col.find_one({"$or": [{"user_id": target_id}, {"id": str(target_id)}, {"id": f"h_{target_id}"}, {"_id": f"host_{target_id}"}]})
                user = users_col.find_one({"user_id": target_id})
                if host:
                    ...
                    msg_text = f"❌ Host ID <code>{target_id}</code> approved ya registered nahi mila!"
                send_telegram_message(chat_id, msg_text)
            except Exception:
