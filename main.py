...
def get_hosts():
    ...
    dummy_hosts = [
        ...
        },
        {
            ...
            "bio": "Parisian fashion enthusiast",
            "is_online": True,
            "is_dummy": True
        },
        {
            ...
            "bio": "Professional singer & artist",
            "is_online": True,
            "is_dummy": True
        },
        {
            ...
            "lang": "Hindi",
            "loc": "Delhi",
            "img": "https://images.unsplash.com/photo-1529626455594-4ff0802cfb7e?w=400&auto=format&fit=crop",
            "bio": "Friendly companion & gamer",
            "is_online": True,
            "is_dummy": True
        }
    ]
...
def book_slot(data: BookingModel):
    if str(data.host_id).startswith("dummy_"):
        return {
            "status": "error",
            "message": "Host is currently busy in a private international session. Please try another host!"
        }
    user = users_col.find_one({"user_id": int(data.user_id)})
    if not user or user.get("tokens", 0) < data.token_cost:
        return {
            "status": "error",
            "message": f"Insufficient tokens! You need {data.token_cost} tokens. Please recharge."
        }
    ...
    try:
        ...
        for b in all_host_bookings:
            ...
            if b.get("status") == "pending" and start_time > 0 and (now - start_time) > 600:
                ...
                if host_tg_id:
                    send_telegram_message(host_tg_id, f"Booking request from User {b['user_id']} expired because you didn't accept it within 10 minutes.")
    ...
    if "message" in body:
        ...
        if text.startswith("/ban "):
            ...
            try:
                ...
                send_telegram_message(target_id, f"Admin deducted {amt} Tokens from your wallet.")
            except Exception as e:
