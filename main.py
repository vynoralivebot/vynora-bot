...
def get_host_status(user_id: int):
    host = hosts_col.find_one({"user_id": int(user_id)})
    if host:
...
def toggle_host_live(user_id: int):
    host = hosts_col.find_one({"user_id": int(user_id), "status": "approved"})
    if not host:
        return {"status": "error", "message": "Host not found or not approved"}
    new_status = not host.get("is_online", False)
    hosts_col.update_one({"user_id": int(user_id)}, {"$set": {"is_online": new_status}})
...
def book_slot(data: BookingModel):
    ...
    booking_doc = {
        ...
        "status": "pending",
        "time": current_time,
        "call_started_at": 0  # Timer starts ONLY when host accepts call
    }
    bookings_col.insert_one(booking_doc)
    
    host = hosts_col.find_one({"user_id": int(clean_host_id)}) if clean_host_id.isdigit() else None
    if host:
        host_telegram_id = host.get("user_id")
        if host_telegram_id:
            keyboard = {
                "inline_keyboard": [
                    [{"text": "✅ Accept & Join", "callback_data": f"accept_bk_{booking_id}"}, {"text": "❌ Reject", "callback_data": f"reject_bk_{booking_id}"}]
                ]
            }
...
def get_host_bookings(user_id: int):
    try:
        ...
        bookings_cursor = bookings_col.find({"host_id": {"$in": h_ids}}).sort("time", -1).limit(50)
        host_bookings = []
        for b in bookings_cursor:
            call_time_formatted = time.strftime('%Y-%m-%d %H:%M', time.localtime(b.get("time", time.time())))
            u_info = users_col.find_one({"user_id": int(b.get("user_id", 0))})
            u_name = u_info.get("full_name", f"User {b.get('user_id')}") if u_info else f"User {b.get('user_id')}"
            ...
            host_bookings.append({
                ...
                "call_started_at": b.get("call_started_at", 0),
...
def accept_booking(data: ActionBookingModel):
    ...
    current_time = time.time()
    # Timer and call start accurately upon acceptance
    bookings_col.update_one({"_id": booking["_id"]}, {"$set": {"status": "approved", "call_started_at": current_time}})
    
    clean_h = str(booking["host_id"]).replace("h_", "").replace("host_", "")
    host = hosts_col.find_one({"user_id": int(clean_h)}) if clean_h.isdigit() else None
    ...
    # Notify team groups upon call completion/acceptance for payment tracking
    group_report = f"✅ <b>Call Accepted & Started!</b>\nHost: {booking.get('host_name')}\nUser ID: <code>{booking.get('user_id')}</code>\nDuration: {booking.get('duration_mins')} Mins\nCost: {booking.get('token_cost')} Tokens"
    send_telegram_message(GROUP_1_ID, group_report)
    send_telegram_message(GROUP_3_ID, group_report)
...
def get_user_bookings(user_id: int):
    try:
        user_bookings_cursor = bookings_col.find({"user_id": int(user_id)}, {"_id": 0}).sort("time", -1).limit(50)
        user_bookings = []
        for b in user_bookings_cursor:
            call_time_formatted = time.strftime('%Y-%m-%d %H:%M', time.localtime(b.get("time", time.time())))
            clean_h = str(b.get("host_id")).replace("h_", "").replace("host_", "")
            host = hosts_col.find_one({"user_id": int(clean_h)}) if clean_h.isdigit() else None
            if host:
...
def register_host(data: HostRegisterModel):
    ...
    keyboard = {
        "inline_keyboard": [
            [{"text": "✅ Approve Host", "callback_data": f"approve_host_{data.user_id}"}, {"text": "❌ Reject Host", "callback_data": f"reject_host_{data.user_id}"}]
        ]
    }
        ...
        if text.startswith("/ban "):
            ...
            try:
                ...
                if host:
                    ...
                    msg = (f"📹 <b>Host Profile & Status Info:</b>\n\n🆔 ID: <code>{target_id}</code>\n👤 Name: {host.get('name', 'N/A')}\n📌 Status: <b>{str(host.get('status', 'pending')).upper()}</b>\n🟢 Live Online: {host.get('is_online', False)}\n🪙 Call Rate: {host.get('rate', 50)} Tokens/min\n💰 Total Earnings: {raw_earnings} Tokens\n💸 Net Payout (70%): ₹{net_payout}")
                else:
